...
export default function AdminLoginScreen({ onLoggedIn }) {
  ...
  const submit = async () => {
    ...
    try {
      if (configured) {
        ...
        await api.adminSetup(username.trim(), password);
      }
      onLoggedIn();
    } catch (e) {
  ...
  const cannotSubmit = !checked || !username.trim() || !password || (!configured && password.length < 4);
        ...
        {checked && !configured ? (
          <>
            ...
            <div style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: '1.4rem', lineHeight: 1.6 }}>
              このサーバーではまだ管理者アカウントが設定されていません。ユーザー名とパスワードを決めてください（この画面を最初に開いた人が管理者になります）。
            </div>
          </>
        ) : (
